...
class StreamripQualitySelector:
    ...
    async def _register_handler_and_wait(self):
        ...
        try:
            ...
            LOGGER.warning(f"Quality selection timed out after {self._timeout} seconds")
            # Send timeout message
            from utils.message_utils import send_message, auto_delete_message, delete_message
    ...
    async def _show_quality_selection(self):
        ...
        for quality in available_qualities:
            ...
            button_text = f"🔸 {quality_info['name']} - {quality_info['description']}"
        ...
        # Add subscription requirements info
        msg += "<i>🔒 = Requires subscription</i>\n"
        msg += "<i>👑 = Requires premium subscription</i>\n\n"
        msg += f"<b>⏱️ Timeout:</b> <code>{self._get_readable_time(self._timeout - (time() - self._time))}</code>"
    ...
    async def _finalize_selection(self):
        ...
        from utils.message_utils import edit_message, auto_delete_message
